...
<body>
    ...
    <div class="container-fluid" style="background-color:  #F3672B;">
        <div class="navbar-header" style="background-color:  #F3672B;">
            <a href="index_pengguna.php"><img src="assets/img/loginregis.png" alt="" width="120px" height="120px"></a>

        </div>
        <ul class="nav navbar-nav">
            <br>
            <li><a href="index_pengguna.php" style="color: white;"> <b> Beranda</b></a></li>
            <li><a href="perhitungan.php" style="color: white;"> <b>Pemilihan Mobil </b></a></li>
    ...
    <div id="page-inner" style="padding: 100px; ">
        <div class="row">
            <div class="col-md-12">
                <h1 class="page-head-line"> Hasil Pemilihan Mobil </h1>
            </div>
        </div>
        <div class="col-md-16">
                    ...
                    <div class="datatable table-responsive">
                        <table class="table table-striped table-bordered table-hover">
                            ...
                            <tbody>
                                <?php
                                $nomor = 0;
                                $hasil = mysqli_query($connect, "select * from normal, alternatifhp where normal.Id_mobil=alternatifhp.Id_mobil");
                        ...
                        <div class="datatable table-responsive">
                            <table class="table table-striped table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php
                                    $nomor = 0;
                                    $hasil = mysqli_query($connect, "select * from normal JOIN alternatifhp ON normal.Id_mobil=alternatifhp.Id_mobil where kategori_kendaraan='$bobot_kategori_kendaraan' limit 5");
                                    while ($data = mysqli_fetch_array($hasil)) {
                                    ?>
                                        <tr>
                                            <td><?php echo $nomor = $nomor + 1; ?></td>
                                ...
                                <table class="table table-striped table-bordered table-hover" id="rangking">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Hasil Perangkingan Mobil </th>
...
$query = "
    SELECT 
        ...
        ROUND(
            ...
            ((normal.Kisaran_harga_beli2 / {$min['min3']}) * $bobot_harga_beli) +
    ...
    FROM 
        normal 
    JOIN 
        alternatifhp ON normal.Id_mobil = alternatifhp.Id_mobil 
    WHERE 
        alternatifhp.kategori_kendaraan = '$bobot_kategori_kendaraan' 
    ORDER BY 
        ranks DESC 
    LIMIT 5";
...
while ($data = mysqli_fetch_array($hasil)) {
    $nomor++;
    // Tampilkan data kendaraan dan skor perangkingan dalam tabel
    echo "<tr>
        <td>$nomor</td>
        <td>{$data['Nama']}</td>
        <td>{$data['kategori_kendaraan']}</td>
        <td>{$data['Transmisi']}</td>
        <td>{$data['Kisaran_harga_beli']}</td>
